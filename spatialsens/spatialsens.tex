...
%\url{http://nextcloud.openmole.org/s/HsyqS3bSiS7Tb6B/download}

}

\sframe{Practice: exploration of generators}{

Script \texttt{SpatialSampling.oms}

\medskip

\begin{enumerate}
	\item Use the \texttt{MatrixHook} and an \texttt{EmptyTask}	to visualize different types of grid
	\item Explore the generators applied to the ZOMBIE model at \url{https://om.exmodelo.org/spatialsens/}
	\item Plug a hook to retrieve all flat configurations (using the commented ScalaTask)
...
$\rightarrow$ explore the script \texttt{spatialsens.oms}
...
$\rightarrow$ explore the script \texttt{phasediag.oms}
...
$\rightarrow$ explore the script \texttt{calibrate.oms}
